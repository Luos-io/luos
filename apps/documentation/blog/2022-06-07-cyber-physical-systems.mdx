...
Another example is a power grid. The physical system is the grid itself, and the cyber one is the computer that monitors and controls the grid. The two devices are connected through a [network](../luos-technology/node/topology), which in this case is the wired or wireless connection between the grid and the computer.
