...
Luos is not an operating system. Our technology allows the developers to **encapsulate any embedded system features** into something we call [services](/documentation/luos-technology/services).
...
So, it's easy! We already provided some frameworks, allowing you to deal with Luos using [Python](/documentation/integrations/pyluos), [ROS](/2022-08-16-ros-controlling-your-distributed-robot-using-micro-ros-and-luos.mdx), etc.
...
The only difference is the path to the information, which is the [Luos network](/documentation/luos-technology/node/topology) used to transmit this information. But the information will still contain the same data.
...
Luos uses something we call "HAL": [hardware attraction layer](/documentation/luos-technology/node/luos-hal). The idea is to tell Luos how to use the hardware it is running on. Luos will be able to use this interface to access the network, memory or timers for example.
