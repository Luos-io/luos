...
Let's talk about applications first. I previously explained the differences between a [driver service](/documentation/luos-technology/services/create-services#drivers-guidelines) and an [application service](/documentation/luos-technology/services/create-services#apps-guidelines).
...
But you can share a driver on an application. We call it a [Luos package](/documentation/luos-technology/packages).
...
Many behaviors start directly from an interrupt. When you receive a [message](/documentation/luos-technology/messages), there will be an interruption, and Luos will create some jobs. These will be in the Luos loop you are running on your system.
...
We have added some examples for [ESP32](/tutorials/esp), some of the [gate](/documentation/tools/gate) web circuit examples we are working on right now, and as we don't have something compiling already, this will be integrated into the next release which will be probably the **2.6.0**.
...
Regarding the update capabilities of Luos, because Luos accesses to the network and we also have access to the flash (basically, we need to have these two accesses to make Luos run), you can compare Luos to [a bootloader](/documentation/tools/bootloader).
