...
Let's talk about applications first. I previously explained the differences between a [driver service](/luos-technology/services/create-services#drivers-guidelines) and an [application service](/luos-technology/services/create-services#apps-guidelines).
...
But you can share a driver on an application. We call it a [Luos package](/luos-technology/packages).
...
Many behaviors start directly from an interrupt. When you receive a [message](/luos-technology/messages), there will be an interruption, and Luos will create some jobs. These will be in the Luos loop you are running on your system.
...
We have added some examples for [ESP32](/tutorials/esp), some of the [gate](/tools/gate) web circuit examples we are working on right now, and as we don't have something compiling already, this will be integrated into the next release which will be probably the **2.6.0**.
...
Regarding the update capabilities of Luos, because Luos accesses to the network and we also have access to the flash (basically, we need to have these two access to make Luos run), you can compare Luos as [a bootloader](/tools/bootloader).
...
We also have the `good first issue` tag. This tag applied on an issue means that any user (even those who never used Luos but just want to help this open source initiative) can start they contributing journey with this kind of issue, with easy bugs and improvements. We also have a [documentation page](contribute-to-luos) to help you through your contributions.
