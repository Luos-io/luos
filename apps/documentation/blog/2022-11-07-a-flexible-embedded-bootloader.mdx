...
Each company develops its own [bootloader](/documentation/tools/bootloader), the current operation is that to flash an MCU, it is necessary to connect to the card physically, use an IDE to launch the command, and update the firmware. The update is more complex in the case of several cards of different brands.
