...
A node is a physical component (hardware) running Luos engine and hosting one or several services. In a Luos network, nodes are all connected together using a network interface. You can choose or create your own communication bus, Luos provides <Tooltip def={useDocusaurusContext().siteConfig.customFields.robus_def}>Robus</Tooltip> as the default network layer.
...
<div align="center">
  <Image
    sources={{ light: '/assets/images/MCU-luos.png', dark: '/assets/images/MCU-luos-white.png' }}
    height="200px"
  />
</div>
