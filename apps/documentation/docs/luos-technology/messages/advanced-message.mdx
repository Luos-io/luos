...
**A streaming channel is always created by the strict real-time service.** The other service (the non-real-time one) will just send or receive its data chunks using [large data messages](/documentation/luos-technology/messages/advanced-message#large-data).
