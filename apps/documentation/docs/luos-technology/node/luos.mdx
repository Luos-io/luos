...
To match the Luos engine's library with your hardware, it offers an _Hardware Abstraction Layer_ for various devices in the <Tooltip def={useDocusaurusContext().siteConfig.customFields.luoshal_def}>Luos engine's HAL folder</Tooltip>.
...
Adding this code to a <Tooltip def={useDocusaurusContext().siteConfig.customFields.node_def}>node</Tooltip> makes it able to react to a Luos network. It is now ready to host your services by [running packages](../packages/index.mdx) on your main.
...
<div align="center">
  <Image
    sources={{ light: '/assets/images/NodeFlow.svg', dark: '/assets/images/NodeFlow-dark.svg' }}
  />
</div>
