...
As explained in the [architecture page](../basics/organization), Luos is a platform that handles packages execution.
...
A Luos [node](../node) can host multiple packages, and a package has to be as portable as possible. In order to do that, packages have to be independent code folders that can be easily copied and pasted in another project.
 ...
 > You can find a view of complete product on [Code organization page](../basics/organization).
...
This way, it is easy to manage all your [services](../services) and add as many of them you want into `main()`.
