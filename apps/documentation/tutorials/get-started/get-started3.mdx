...
In this part, we will use the “default wiring” defined by Luos to create a network of two MCUs. We will use a [OneWire network](/docs/test/hardware-consideration/electronics), limiting the circuit to simple wires and pins:

- Tx and Rx pins, which are connected together and wired to the Tx and Rx pins of the other board (see the next image).
- One PTP wire.

:::tip
💡 Luos Point-To-Point (PTP) connection allows to find your board's [physical position](/docs/test/luos-technology/node/topology). Luos engine can deal with up to 8 PTP lines on each board to connect as many boards as you want.
...
You can check out our [tutorials](/tutorials) to learn more about Luos and understand how to use the features of Luos engine. We also invite you to check out our [documentation](/docs/test/luos-technology) to learn more about the core concepts of Luos engine.
