...
At the node level, communication is achieved by receiving and sending [messages](../messages) with the other [services](../services) of a Luos network. The nodes can communicate with each other thanks to the network layer.
