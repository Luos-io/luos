...
Adding this code to a <Tooltip def={customFields.node_def}>node</Tooltip> makes it able to react to a Luos network. It is now ready to host your services by [running packages](../packages) on your main.
...
At the node level, communication is achieved by receiving and sending [messages](../messages) with the other [services](../services/index.mdx) of a Luos network. The nodes can communicate with each other thanks to the network layer.
...
|      NBR_PORT      |           2           | Number of PTP (port) on the node ( max 8). See [electronic design](../../hardware-consideration/electronics.mdx) page. |
