...
Robus uses wires called [PTP]/next/luos-technology/node/topology#ptp-network) to poke the nodes neighbors and see if there is any.

The example below shows the process of assigning a node ID during the topology detection of three [nodes]/next/luos-technology/node). The starting node always has the `node_ID` set to the value 1.
