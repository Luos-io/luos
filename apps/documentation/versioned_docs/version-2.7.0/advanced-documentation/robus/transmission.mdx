...
When `Luos_send` API is called, the message to transmit is stored in the Luos engine's [message]/next/documentation/luos-technology/messages) allocator _msg_alloc.c_.

Depending on the target of this message, Luos engine will pass it to a local service ([localhost](http://localhost) network) or to a [network layer]/next/documentation/luos-technology/node/luos-hal), in this case, Robus.
...
During message reception, Robus will use the message [header]/next/documentation/luos-technology/messages#header) and the information of the node it's running on to know whether it needs to keep this message or not.
